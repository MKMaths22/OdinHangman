...
class Game
  attr_accessor :guesses_remaining, :all_guessed_letters, :incorrect_guessed_letters, :state_of_word, :secret_word,
                :player_name, :game_saved, :solved, :failed, :saved, :save_slot, :reloaded

  ALPHA_REGEX = /^[A-Z]$/.freeze
  ...
  def find_save_slot
    ...
    i = 1
    i += 1 while File.exist?("#{player_name}/#{i}.txt")
    puts "Game saved in your named folder, in slot #{i}."
    self.save_slot = i
  end

  def save_game
    Dir.mkdir(player_name.to_s) unless Dir.exist?(player_name.to_s)
    # make the player's own directory if it does not exist
    saved_back_to = "Game saved in your named folder, back into slot #{save_slot}"
    ...
    file_for_saving = File.new("#{player_name}/#{save_slot}.txt", 'w')
    file_for_saving.puts saved_game_as_yaml
    file_for_saving.close
    self.saved = true
    # the saved value will stop play_hangman from continuing to execute its 'do' loop
    # but does not affect the game that will be reloaded
  end

  def find_if_saves_exist(name)
    Dir.exist?(name) && !Dir.empty?(name) ? choose_reload(name) : play_hangman
  end
  ...
  def score_incorrect_guess(letter)
    ...
    incorrect_guessed_letters.push(letter)
    ...
    puts @guesses_remaining.zero? ? solved_fail : letter_fail
  end
  ...
  def score_correct_guess(letter, num)
    (0..secret_word.length - 1).each do |i|
      state_of_word[i] = secret_word[i] if secret_word[i] == letter
    end
  ...
  def make_a_guess
    ...
    all_guessed_letters.push(letter_chosen)
    number_of_hits = secret_word.count(letter_chosen)
    score_incorrect_guess(letter_chosen) if number_of_hits.zero?
    self.failed = true if guesses_remaining.zero?
    score_correct_guess(letter_chosen, number_of_hits) if number_of_hits.positive?
...
name = gets.strip
game = Game.new(name)
puts "Nice to see you again, #{name}!" if Dir.exist?(name)
